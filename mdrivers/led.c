...
int32_t ledRegister(ledDriver_t *driver, uint32_t to){
    ...
    if( lock && (lock(to) != 0) ) return LED_ERROR_LOCK;

    leds[n] = *driver;
    idx = (int32_t) n;
    n++;

    if( unlock && (unlock() != 0) ) return LED_ERROR_UNLOCK;
...
int32_t ledSetIntensity(int32_t idx, void*p, uint8_t intensity, uint32_t to){
    ...
    if( leds[idx].setIntensity ) leds[idx].setIntensity(p, intensity);
...
int32_t ledSetColor(int32_t idx, void*p, uint8_t r, uint8_t g, uint8_t b, uint32_t to){
    ...
    if( leds[idx].setColor ) leds[idx].setColor(p, r, g, b);
